...
class DataPreparationModule:
    ...
    def load_documents(self) -> List[Document]:
        ...
        for md_file in data_path_obj.rglob("*.md"):  # 递归查找所有Markdown文件(../../data/C8/cook 路径下)
            try:
                ...
                try:
                    data_root = Path(self.data_path).resolve()  # 获取数据根目录的绝对路径 WindowsPath('D:/all-in-rag/data/C8/cook')
                    relative_path = Path(md_file).resolve().relative_to(data_root).as_posix()  # 计算当前文件相对于根目录的相对路径 'dishes/aquatic/咖喱炒蟹.md'
                except Exception:
                    relative_path = Path(md_file).as_posix()
                parent_id = hashlib.md5(relative_path.encode("utf-8")).hexdigest()  # 对相对路径做 MD5 哈希，生成固定长度的唯一ID '74ce7655f477d8c91f22a170e30acd3a'
    ...
    def _enhance_metadata(self, doc: Document):
        ...
        path_parts = file_path.parts  # ('..', '..', 'data', 'C8', 'cook', 'dishes', 'aquatic', '咖喱炒蟹.md')
