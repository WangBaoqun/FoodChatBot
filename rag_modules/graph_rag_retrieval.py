        ...
        try:
            with self.driver.session() as session:
                ...
                for record in result:
                    ...
                    # 按节点度数降序排列的字典 key=node_id
                    self.entity_cache[node_id] = {
                        "labels": record["node_labels"],  # 节点类型 例如 ['Recipe']
                        "name": record["name"],  # 名称 例如 '咖喱炒蟹'
                        "category": record["category"],  # 分类 例如 '水产'
                        "degree": record["degree"]  # 节点的度 例如 81
                    }
